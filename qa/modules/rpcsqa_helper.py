...
class rpcsqa_helper:
    ...
    def install_cookbooks(self, chef_server, url, branch,
        ...
        run_cmd = self.run_command_on_node(chef_server, command)
        if not run_cmd['success']:
            print "Command: %s failed to run on %s" % (command, chef_server)
            print run_cmd
            sys.exit(1)
        ...
        for cmd in cmds:
            run_cmd = self.run_command_on_node(chef_server, cmd)
            if not run_cmd['success']:
    ...
    def bootstrap_chef(self, name, server_node):
        ...
        ssh_run = self.run_command_on_node(Node(server_node), cmd)
