...
parser.add_argument('--policy', action="store", dest="policy", required=True,
                    help="Razor policy to set chef roles for.")
parser.add_argument('--tempest_dir', action="store", dest="tempest_dir",
                    required=True,
                    default="/var/lib/jenkins/tempest/folsom/tempest",
                    help="Directory of tempest files")
...
parser.add_argument('--chef_url', action="store", dest="chef_url",
                    default="http://198.101.133.4:4000", required=False,
...
parser.add_argument('--display_only', action="store", dest="display_only",
                    ...
                    help="Display the node information only")
results = parser.parse_args()


def get_chef_name(data):
    try:
        name = "%s%s.%s" % (data['hostname_prefix'],
                            data['bind_number'],
                            data['domain'])
        return name
    except Exception:
        # should probably do something here fools (jwagner)
        return ''
...
if active_models:
    # Gather all of the active models for the policy and
    ...
    for active in active_models:
        ...
        with ChefAPI(results.chef_url,
                     results.chef_client_pem,
                     results.chef_client):
            node = Node(chef_name)
